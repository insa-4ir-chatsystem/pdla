...
public class DBManager {
	...
	private static final String URL = "jdbc:mysql://srv-bdens.insa-toulouse.fr:3306/projet_gei_026"; // Remplacez par l'URL de votre base de données
	...
	public boolean printQuery(String sql, Object... params) throws SQLException {
	    ...
	    try (
	        ...
	        try (ResultSet rs = pstmt.executeQuery()) {
	        	...
	        	if (!FoundMission) return FoundMission ;
	        	else {
		            ...
		            while (rs.next()) {
		                for (int i = 1; i <= columnCount; i++) {
		                    String value = rs.getString(i);
		                    if (value == null) {
		                        value = "NULL"; // Si la valeur est NULL, afficher "NULL"
		                    }
	...
	public void printTable(String TableName) throws SQLException {
        ...
        try {
            ...
            // Revenir au début du ResultSet après avoir ajusté les largeurs
            rs.beforeFirst(); // Cette ligne ne posera plus de problème
            ...
            if (rowsInserted > 0) {
                ...
                try (ResultSet generatedKeys = pstmt.getGeneratedKeys()) {
                    if (generatedKeys.next()) {
                    	user.setID(generatedKeys.getInt(1)) ; // changer l'id du USER pour qu'il corresponde à celui de la table
                        System.out.println("¤ L'utilisateur " + user.getFirstName() + " " + user.getLastName() + " a été ajouté avec succès ! ¤") ;
                    } else {
    ...
    public void addMission(Mission mission) throws SQLException {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql,Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (mission.getBenevole() != null) {
                ...
                pstmt.setNull(6, java.sql.Types.INTEGER); // Utilisation de setNull pour benevole_id
            }

            // Si valideur est null, on passe NULL à la base de données
            if (mission.getValideur() != null) {
                pstmt.setInt(7, mission.getValideur().getID());
            } else {
                pstmt.setNull(7, java.sql.Types.INTEGER); // Utilisation de setNull pour valideur_id
            }
            ...
            if (rowsInserted > 0) {
                ...
                try (ResultSet generatedKeys = pstmt.getGeneratedKeys()) {
                    if (generatedKeys.next()) {
                    	mission.setId(generatedKeys.getInt(1)) ; // changer l'id de la mission pour qu'il corresponde à celui de la table
                        System.out.println("¤ La mission de nature : " + mission.getNature() + " a été ajouté avec succès ! ¤") ;
                    } else {
    ...
    //changer la modification des donnée du user retrouvé 
    public boolean authenticateUser(User user, String email, String password) throws SQLException {
        ...
        if (rs.next()) {
            // Si un utilisateur correspondant est trouvé, on remplit l'objet User avec les données récupérées
